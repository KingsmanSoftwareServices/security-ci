...
Both checks scan the current dependency tree on every pull request. Keep the initial
thresholds only as long as needed to make the existing backlog satisfiable, and review
dependency changes carefully while ratcheting them down.
...
**Private repositories on the Free plan** need no special security inputs. The workflow
does not call GitHub Advanced Security or a third-party paid runner-hardening service;
scanner findings are kept in the run's downloadable artifacts.
