...
When **PR Malware Gate**, Gitleaks, OSV, Semgrep, the dependency audit, or the post-build scanner fails:
...
3. Inspect the finding from the GitHub Actions log using the file/rule name, and from the uploaded `*-report` artifact on the run (JSON + SARIF, retained 14 days). The custom scanner intentionally does not print suspect payload contents or secrets.
