...
The pipeline does not require GitHub Advanced Security or a paid third-party security
service. It uses local/open-source scanners and stores JSON/SARIF output as ordinary
workflow artifacts, so the same jobs run in private repositories on GitHub Free (subject
to the account's normal Actions minutes and storage allowance).
